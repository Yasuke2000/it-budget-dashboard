      ...
      {rows.map((r, i) => (
        <div key={`${r.co}-${r.doc}-${i}`} className="flex items-baseline justify-between gap-2 border-b border-border/40 py-0.5 text-[11px]">
          <span className="min-w-0 truncate">
            ...
            {r.factored && <span className="ml-1 rounded bg-muted px-1 text-[9px] font-semibold text-muted-foreground ring-1 ring-border" title="Actief gefactorde klant (WHS/KBC): 90% al voorgeschoten, alleen het 10%-saldo telt in de met-factoring-reeks">factor</span>}
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <Kpi label={zonderVerleden ? "Laagste punt (ritme, zonder achterstal)" : "Laagste punt (kasrealiteit)"} value={eurS(lowWithF!.value)} tone={lowWithF!.value < 0 ? "neg" : "pos"}
              ...
              onClick={() => setKpiSrc({ label: "Laagste punt — kasrealiteit", value: eur(lowWithF!.value), bron: "Cumulatief saldo per week mét factoring: bestaande posten (actief gefactorde WHS/KBC-klanten alleen het 10%-saldo — 90% is al binnen via de 433; andere vennootschappen 100%) + nieuwe facturatie op weekritme (90% ±1 week na uitreiking) − leveranciers − lonen/btw/leasing − nieuwe inkopen." + (hasAdj ? " In deze weergave tellen óók de actieve prognose-aanpassingen (scenario-invoer, blok onderaan de pagina) mee." : ""), caveat: "Rood = financieringsbehoefte, niet lege kas: kredietlijnen/straight loans zitten er bewust niet in." })} />
            ...
            <Kpi label="433-saldo (factor R/C)" value={eurS(d.totals.saldo433)} sub="de lump-sum 'zak met geld' bij de factors"
              onClick={() => setKpiSrc({ label: "Rekening 433 — factor rekening-courant", value: eur(d.totals.saldo433), bron: "trialBalances per vennootschap, alle 433-rekeningen: het saldo tussen voorgeschoten (90%, WHS/KBC) en afgerekende facturen. Negatief = opgenomen voorschot (schuld aan de factor).", caveat: "Koppeling van individuele 433-bewegingen aan facturen is fase 2 (factorportaal-rapporten)." })} />
          </div>
          ...
          {(() => {
            ...
            const brugDetail: Record<string, React.ReactNode> = {
              ...
              inNieuw: <p>Raming, geen individuele posten: facturatie loopt door op het gemiddelde weekritme van de laatste 12 volle weken{scenario !== "zonder" ? "; bij actief gefactorde klanten (WHS/KBC) komt 90% ±1 week na uitreiking binnen, de rest op betaalgedrag" : ", geïnd op betaalgedrag"}. Dit is het ritme-deel van het model.</p>,
              ...
              <table className="w-full min-w-[1060px] text-[11px]">
                <thead>
                  <tr className="border-b border-border text-left text-[10px] uppercase tracking-wide text-muted-foreground">
                    ...
                    <th className="py-1.5 pr-2 text-right" title="bestaande posten (10%-saldo bij WHS/KBC) + nieuwe facturatie: 90% ~1 week na uitreiking + 10% op betaalgedrag">In kasrealiteit</th>
