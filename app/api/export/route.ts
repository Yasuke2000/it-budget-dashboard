...
${(() => {
  // Build a lookup of invoice-based category variance to suppress contradictory budget_overrun insights
  const catVariance = new Map(categories.map(c => [c.category, c.variancePercent]));

  const filtered = insights.filter((i) => {
    if (i.category !== "budget_overrun") return true;
    // Extract category name from the insight title (format: "Category: X% over budget...")
